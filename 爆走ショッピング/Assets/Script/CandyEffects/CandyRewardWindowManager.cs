...
public sealed class CandyRewardWindowManager : MonoBehaviour
{
    ...
    [SerializeField] private string libraryResourcePath = "CandyEffectLibrary";
    ...
    private void ResolveReferences()
    {
        ...
        if (inventoryInfluenceSettings == null)
        {
            inventoryInfluenceSettings = FindAnyObjectByType<InventoryInfluenceSettings>();
        }
        ...
        if (candyEffectLibrary == null && loadLibraryFromResources)
        {
            LoadCandyEffectLibrary();
        }
    }

    private void LoadCandyEffectLibrary()
    {
        if (!string.IsNullOrWhiteSpace(libraryResourcePath))
        {
            candyEffectLibrary = Resources.Load<CandyEffectLibrary>(libraryResourcePath);
        }
        ...
        CandyEffectLibrary[] libraries = Resources.LoadAll<CandyEffectLibrary>(string.Empty);

        if (libraries != null && libraries.Length > 0)
        {
            candyEffectLibrary = libraries[0];
        }
    }

    private void SubscribeScoreboard()
    {
        if (scoreboardManager == null)
        {
            return;
        ...
        scoreboardManager.SettlementCompleted -= HandleSettlementCompleted;
        scoreboardManager.SettlementCompleted += HandleSettlementCompleted;
    }

    private void UnsubscribeScoreboard()
    {
        if (scoreboardManager == null)
        {
            return;
